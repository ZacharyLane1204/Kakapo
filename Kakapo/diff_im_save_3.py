...
class Difference_Imaging():
    ...
    def compute_difference_images_with_psf(self):
        ...
        for i in range(len(self.flux)):
            ...
            diff_clean, diff_sig = self._computing_difference_image(self.flux[i], self.dxs[i], self.dys[i], self.true_noise[i])
    ...
    def _computing_difference_image(self, flux, dx, dy, true_noise):
    ...
    def _background_internal(self, frame, noise, snr_thresh = 2.5):
        ...
        bkg_var = np.clip(np.abs(bkg), a_min=1.0, a_max=None) / self.exptime + np.abs(bkg_err)**2
