...
class Difference_Imaging():
    ...
    def build_reference(self):
        ...
        ref, init_ref_noise = self.refine_reference(shifted_stack, noise_stack, temp_ref, 
                                                    temp_init_ref_noise, snr_thresh=3.0, huber_delta=2.0)
        # ref = temp_ref.copy()
        # init_ref_noise = temp_init_ref_noise.copy()
        
        # transient_mask = self._psf_matched_mask(ref, self.psf, init_ref_noise, snr_thresh=1, dilate_radius=1)
        bkg_mask = ~(self.persistent_mask)
        
        bkg_result = self._fit_background_plane_new(ref, mask = bkg_mask)
        
        bkg = bkg_result['background']
        bkg_rms = bkg_result['bg_rms']
        
        poisson_variance_ref = np.clip(np.abs(ref), a_min=1.0, a_max=None) #/ self.exptime
        bkg_variance_ref = np.clip(np.abs(bkg), a_min=1.0, a_max=None) + bkg_rms**2 #/ self.exptime + 
        true_bkg_variance_ref = np.clip(np.abs(bkg), a_min=1.0, a_max=None) / self.exptime + (bkg_rms/self.exptime)**2
    ...
    def _minimisation_routine(self):
        ...
        for i in range(len(self.flux)):
            ...
            transient_mask = self._psf_matched_mask(self.flux[i], self.psf, 
                                                   self.noise[i], snr_thresh=1.5, dilate_radius=1)
            
            bkg_mask = ~(self.persistent_mask | transient_mask)

            bkg_result = self._fit_background_plane_new(self.flux[i], mask = bkg_mask)
            bkg_init = bkg_result['background']
            bkg_init_rms = bkg_result['bg_rms']

            self.flux[i] -= bkg_init
            ...
            #                                                     np.nan_to_num(self.flux[i] * tukeying), 
            #                                                     upsample_factor=200)
            
            initial_sci_im = self._shift_fourier(self.flux[i], initial_shift_guess[1], initial_shift_guess[0])
            
            initial_diff = (initial_sci_im - self.ref)
            
            transient_mask = self._psf_matched_mask(initial_diff, self.psf, np.sqrt(self.noise[i]**2 + bkg_init_rms**2), 
                                                    snr_thresh=1.5, dilate_radius=1)
            
            bkg_mask = ~(self.persistent_mask | transient_mask)
            
            bkg_result = self._fit_background_plane_new(initial_diff, mask = bkg_mask)
            bkg = bkg_result['background']
            bkg_rms = bkg_result['bg_rms']
            
            bkg_var = np.clip(np.abs(bkg) + np.abs(bkg_init), a_min=1.0, a_max=None) / self.exptime
            bkg_var_list.append(bkg_var + bkg_init_rms**2 + bkg_rms**2)
            
            self.flux[i] -= bkg
            
            (dx, dy), chi2 = self.compute_shift(self.flux[i], np.sqrt(self.noise[i]**2 + bkg_var*self.exptime), 
                                                initial_shift_guess, self.persistent_mask, self.bkg_factors[i])
    ...
    def compute_shift(self, frame, noise_frame, initial_shift_guess, transient_mask, bkg_factor):

        def cost_fn(shift_params):
            return self._cost_function_safe(shift_params, frame, noise_frame, transient_mask, bkg_factor)
    ...
    def _cost_function_zogy(self, shift_params, flux_frame, flux_noise_frame, transient_mask, bkg_factor):
        """
        ...
        ref = self.ref.copy() # Reference frame and noise
        ref_noise = self.ref_noise
        
        tukeying = self._tukey2d(*ref.shape, alpha=0.5)
        
        transient_mask = transient_mask.astype(float)
        
        sigma_add = self._sigma_from_motion(dx, dy)

        denom = np.sqrt(noise_shifted + ref_noise**2 + sigma_add**2 + 0.05**2 + 1e-12) * bkg_factor
        D = (frame_shifted - ref) / denom * tukeying * transient_mask

        nu = 8
        robust = np.nansum((nu + 1) / 2.0 * np.log1p((D)**2 / nu))
        
        # robust = np.nansum(self._huber_cost(D, delta=2.0))
    ...
    def compute_difference_images_with_psf(self):
        ...
        np.save('noise_model.npy', np.array(diff_noises))
    ...
    def _fit_background_plane_new(self, image, mask=None):
        ...
        lam = 1e-3 # Regularize slope terms to avoid overfitting tilt in tiny cutouts
        AtA = A.T @ A + lam * np.eye(3)
        Atz = A.T @ z
        coeffs = np.linalg.solve(AtA, Atz)
        
        background = coeffs[0]*x_grid + coeffs[1]*y_grid + coeffs[2] # 5. full background plane (2D image, not just at mask points)

        model_vals = coeffs[0]*x + coeffs[1]*y + coeffs[2] # 6. residuals only at the good pixels
    ...
    def _computing_difference_image(self, flux, dx, dy, bkg_var_arr, true_noise):
        ...
        true_ref_noise = self.true_ref_noise**2

        sigma_add = self._sigma_from_motion(dx, dy)
        ref_var_c = self.safe_fftconvolve(true_ref_noise, self.psf**2)
        frm_var_c = self.safe_fftconvolve(true_frame_err**2 + sigma_add**2, self.psf**2)
        
        diff_sig  = np.sqrt(ref_var_c + frm_var_c)
        diff_sig[diff_sig < 1e-12] = 1e-12
    ...
    def _sigma_from_motion(self, dx, dy, k_xy=0.3, cap=3.0):
        # k_xy maps pixel shift amplitude to intra-exposure blur (px); tune on real data
        r = np.hypot(dx, dy)
        return float(np.clip(k_xy * r, 0.0, cap))

    def _make_star_core_mask(self, psf, frac=0.5):
        # mask inside where PSF cumulative sum reaches `frac` of total flux
        p = psf / np.nansum(psf)
        cy, cx = np.array(p.shape)//2
        yy, xx = np.indices(p.shape)
        r = np.hypot(yy-cy, xx-cx)
        order = np.argsort(r.ravel())
        cs = np.cumsum(p.ravel()[order])
        r_star = r.ravel()[order][np.searchsorted(cs, frac)]
        return (r <= r_star)
