...
class Difference_Imaging():
    ...
    def create_diff_image(self):
        ...
        poisson_variance_stack = np.clip(np.abs(self.flux), a_min=0.0, a_max=None) / self.exptime**2
    ...
    def build_reference(self):
        ...
        poisson_variance_ref = np.clip(np.abs(ref), a_min=0.0, a_max=None) / self.exptime**2
        bkg_variance_ref = np.clip(np.abs(bkg), a_min=0.0, a_max=None) / self.exptime**2
