...
def detrend(dataset,campaign=5,splits=None,quiet=False,save_dir='.',seed=0,flux_type='pdc',default_position_kernel='SqrExp',
           ...
           ls_max_fap=-50,ls_min_period=0.05,ls_max_period=25,  outlier_sigma=5,outlier_mwidth=25, verbose = False):
    ...
    for iset in range(ds.nsets):
        ...
        # full_position = full_like(ds.fluxes[iset], nan)
        full_position = tp + mp
        
        # print('ZZZ:', len(full_position))

        # Full-length time trend
        # full_time = full_like(ds.fluxes[iset], nan)
        full_time = tt + mt

        result = Result(detrender, pv, full_time, full_position, cdpp_r, cdpp_t, cdpp_c, warn)
        if verbose:
        ...
        # full_flux = full_like(ds.fluxes[iset], nan)
        # full_flux[dd.mask] = dd.unmasked_flux
        return result, detrender#, full_flux
...
class k2sc_lc():
    ...
    lc.k2sc()
    '''
    def __init__(self, targetid, time, flux, flux_err, xs, ys, quality, campaign, cadenceno, hdu):
    ...
    def k2sc(self, **kwargs):
        ...
        # print('ZZZ', len(self.tr_position))
        ...
        return self.corr_flux-self.tr_time + nanmedian(self.tr_time)